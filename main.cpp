...
void move_to_bucket(int ball_i, int destination_bucket)
{
    ...
    //move completed
    if (abs(int(c[ball_i].x - destX)) < 0.3 && abs(int(c[ball_i].y - destY)) < 0.3)
    {
        moving = 0;
...
void bringBallsBackDown()
{
    ...
    else//choose ball in bucket remeber copy into circletemp then at end after motion copy circletemp to circle
    {
        ...
        if (c[b[bringBallsDownBucket_i].ballNumberStored[bringBallsDownBallCount_i]].y > 450)
            ...
            c[b[bringBallsDownBucket_i].ballNumberStored[bringBallsDownBallCount_i]].y -= 0.5;
        if (abs(int(c[b[bringBallsDownBucket_i].ballNumberStored[bringBallsDownBallCount_i]].x - cTemp[bringBallsDownDestBall_i].x)) < 0.3 && abs(int(c[b[bringBallsDownBucket_i].ballNumberStored[bringBallsDownBallCount_i]].y - cTemp[bringBallsDownDestBall_i].y)) < 0.3)//move completed
        {
            isMovingBallDown = 0;
